#!/usr/bin/env python3
"""
Test suite for NPC Conversational Orchestrator.

Simulates a tavern interaction where a "dark-shaped figure" NPC
reacts to player greetings and questions about the universe.
"""
...
from experiments.npc_orchestrator import (
    NPCOrchestrator,
    NPCInput,
    NPCOutput,
    Persona,
...
def test_relationship_tracker():
    """Test relationship score tracking with rude/helpful patterns."""
    print("\n" + "=" * 60)
...
def test_tavern_interaction_streaming():
    """
    Test full tavern interaction with the dark-shaped figure NPC.
    Uses streaming mode to test token-by-token generation.
    """
    print("\n" + "=" * 60)
...
def test_interruption_handling():
    """Test that ongoing generation can be interrupted."""
    print("\n" + "=" * 60)
...
def test_social_dynamics():
    """Test that NPC behavior changes based on player rudeness/helpfulness."""
    print("\n" + "=" * 60)
...
def test_memory_serialization():
    """Test that conversation history can be serialized (unit test version)."""
    print("\n" + "=" * 60)
...
def test_data_contract_interface():
    """Test the full NPCInput -> NPCOutput data contract."""
    print("\n" + "=" * 60)
